...
const download = async (req: NextApiRequest, res: NextApiResponse) => {
	...
	const buffer_1 = await kbsFile.xlsx.writeBuffer();

	const oldVersionWorkbook = XLSX.read(buffer_1, { type: 'buffer' });
	...
	for (const courseCode of usedCourseCodes) {
		...
		sheet.getRow(2).getCell(3).font = { size: 12, family: 1, name: 'Times New Roman' };
		...
		sheet.getRow(3).getCell(3).font = { size: 12, family: 1, name: 'Times New Roman' };
		...
		sheet.getRow(4).getCell(3).font = { size: 12, family: 1, name: 'Times New Roman' };
		...
		sheet.getRow(5).font = { size: 12, bold: true, family: 1, name: 'Times New Roman' };
		...
		sheet.getRow(6).font = { size: 12, bold: true, family: 1, name: 'Times New Roman' };
		...
		for (let i = 0; i < documentRowsFiltered.length; i++) {
			sheet.getRow(i + 7).font = { size: 12, family: 1, name: 'Times New Roman' };
		...
		sheet.mergeCells(8 + documentRowsFiltered.length, 2, 8 + documentRowsFiltered.length, 15);
	...
	const buffer_2 = await filesForPrint.xlsx.writeBuffer();

	const zip = new JSZip();

	zip.file(
		`KBS-BELGELER-${documentData.month + 1}-${documentData.year}-${Math.floor(
			Math.random() * 100000,
		)}.xls`,
		oldVersionBuffer,
		{ binary: true },
	...
	zip.file(
		`CIZELGE-${documentData.month + 1}-${documentData.year}-${Math.floor(
			Math.random() * 100000,
		)}.xlsx`,
		buffer_2,
		{ binary: true },
	...
	res.setHeader(
		'Content-Disposition',
		`attachment; filename="KBS-BELGELER-${documentData.month + 1}-${
			documentData.year
		}-${Math.floor(Math.random() * 100000)}.zip"`,
	);
